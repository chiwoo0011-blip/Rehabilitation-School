...
const StopFormScreen = ({ route, navigation }) => {
    ...
    const parseHour = (t) => t ? parseInt(t.split(':')[0], 10) || 8 : 8;
    const parseMin = (t) => t ? parseInt(t.split(':')[1], 10) || 0 : 0;
                ...
                <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
                    ...
                    <View style={styles.timePickerRow}>
                        <View style={styles.timeUnit}>
                            <TouchableOpacity style={styles.timeArrowBtn} onPress={() => setTimeHour((h) => (h + 1) % 24)}>
                                <Ionicons name="chevron-up" size={18} color={COLORS.primary} />
                            </TouchableOpacity>
                            <View style={styles.timeValueBox}>
                                <Text style={styles.timeValueText}>{String(timeHour).padStart(2, '0')}</Text>
                            </View>
                            <TouchableOpacity style={styles.timeArrowBtn} onPress={() => setTimeHour((h) => (h - 1 + 24) % 24)}>
                                <Ionicons name="chevron-down" size={18} color={COLORS.primary} />
                            </TouchableOpacity>
                        </View>
                        <Text style={styles.timeColon}>:</Text>
                        <View style={styles.timeUnit}>
                            <TouchableOpacity style={styles.timeArrowBtn} onPress={() => setTimeMinute((m) => (m + 5) % 60)}>
                                <Ionicons name="chevron-up" size={18} color={COLORS.primary} />
                            </TouchableOpacity>
                            <View style={styles.timeValueBox}>
                                <Text style={styles.timeValueText}>{String(timeMinute).padStart(2, '0')}</Text>
                            </View>
                            <TouchableOpacity style={styles.timeArrowBtn} onPress={() => setTimeMinute((m) => (m - 5 + 60) % 60)}>
                                <Ionicons name="chevron-down" size={18} color={COLORS.primary} />
                            </TouchableOpacity>
                        </View>
...
const styles = StyleSheet.create({
    ...
    timeValueText: { fontSize: 24, fontWeight: 'bold', color: COLORS.primary, fontVariant: ['tabular-nums'] },
    timeColon: { fontSize: 24, fontWeight: 'bold', color: COLORS.primary },
    timePreview: {
