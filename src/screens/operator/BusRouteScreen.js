...
import { subscribeStops, deleteStop, saveStop, initDefaultStops, cleanUpDuplicateSpecials } from '../../services/routeService';
...
const BusRouteScreen = ({ navigation }) => {
    ...
    useEffect(() => {
        // Run cleanup first so the UI instantly reflects the sanitized state
        cleanUpDuplicateSpecials(selectedBus, selectedDir).catch(console.warn);
